#!/usr/bin/env bash
# fnl2: final capacity suite on the compact K/Q MQAR format (data/mqar/gen_fnl2.py).
...
#
# Usage:
#   SLURM=1 bash scripts/sweep_fnl2.sh                    # 2 x 4 x 4 = 32 jobs
#   MIXERS=softmax KS="16 32" SLURM=1 bash scripts/sweep_fnl2.sh
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; cd "$ROOT"

MIXERS_STR="${MIXERS:-softmax gdn}"; read -r -a MIXERS_A <<< "$MIXERS_STR"
KS_STR="${KS:-16 32 64 128}"; read -r -a KS_A <<< "$KS_STR"
LRS_STR="${LRS:-1e-4 4.6416e-4 1e-3 2.1544e-3 1e-2}"; read -r -a LRS_A <<< "$LRS_STR"
...
TIME="${TIME:-12:00:00}"
LOGDIR="slurm/fnl2"; mkdir -p "$LOGDIR"
...
for MIXER in "${MIXERS_A[@]}"; do
  ...
  for K in "${KS_A[@]}"; do
    Q=$K; SEQLEN=$(( 2*K + 2*Q )); DATASET="mqar_fnl2_k${K}_q${Q}"
    [[ -d "data/${DATASET}" ]] || { echo "missing data/${DATASET} (run: K=${K} python data/mqar/gen_fnl2.py)"; exit 1; }
    for LR in "${LRS_A[@]}"; do
      tag="fnl2_${MIXER}_k${K}_lr${LR}"
      log="${LOGDIR}/${tag}.log"; out_dir="out/${tag}"; jobname="fnl2-${MIXER}-k${K}-${LR}"
...
echo "fnl2 launched (${MIXERS_A[*]}). Check: squeue -u \$USER | grep fnl2-"
