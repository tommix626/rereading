#!/bin/bash
# Single-GPU softmax-attention training on the CSAIL CL Slurm cluster.
#   Submit from kennel:  sbatch scripts/train_transformer.sh [CONFIG]
#   Default CONFIG = config/ar_transformer.py (MQAR associative-recall demo).
#
...
cd "$PROJECT"
CONFIG="${1:-config/ar_transformer.py}"
