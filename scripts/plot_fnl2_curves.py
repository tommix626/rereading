"""fnl2 plots: compact K/Q MQAR, softmax(RoPE) vs GDN(noconv), sweep K x LR.

Reads slurm/fnl2/*.log lines `step N: val/loss X val/acc Y`.
Outputs (slurm/plots/):
  fnl2_acc_vs_K.png       -- best-over-LR final val acc vs K (the capacity money plot)
  fnl2_all_lr_curves.png  -- grid [K x mixer], every LR's val-acc curve

Usage: python scripts/plot_fnl2_curves.py
"""
import os, re, glob
...
LOGDIR = os.path.join(ROOT, "slurm", "fnl2")
OUTDIR = os.path.join(ROOT, "slurm", "plots"); os.makedirs(OUTDIR, exist_ok=True)

KS = [int(k) for k in os.environ.get("FNL2_KS", "16 32 64 128 256 512 1024").split()]
MIXERS = ["softmax", "gdn"]
...
FNAME = re.compile(r"fnl2_(softmax|gdn)_k(\d+)_lr([0-9.eE+-]+)\.log$")
LRC = plt.cm.viridis
...
def plot_acc_vs_K(data):
    ...
    plt.ylim(-0.02, 1.02)
    plt.title("fnl2: compact K/Q MQAR — softmax(RoPE) vs GDN(noconv)\n2L d=64, 400k ex, 16 ep, batch 256, 25k steps")
    plt.legend(loc="lower left", fontsize=9); plt.grid(alpha=0.3); plt.tight_layout()
    out = os.path.join(OUTDIR, "fnl2_acc_vs_K.png"); plt.savefig(out, dpi=130); plt.close()
    return out
...
def plot_curves(data):
    ...
    fig.suptitle("fnl2 all-LR curves — softmax(RoPE) vs GDN(noconv), compact K/Q MQAR", y=0.995)
    fig.tight_layout(rect=[0, 0, 1, 0.98])
    out = os.path.join(OUTDIR, "fnl2_all_lr_curves.png"); fig.savefig(out, dpi=120); plt.close(fig)
    return out


def summary(data):
    print("\n=== fnl2 best final val acc (over LR) ===")
